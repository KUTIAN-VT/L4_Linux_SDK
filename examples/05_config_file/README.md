...
`l4_config_file` 用于演示基带配置文件的导出、导入、恢复默认配置和设备重启。它覆盖以下命令，并可通过 `BB_REMOTE_IOCTL_REQ` 在对端执行，远程模式默认使用 slot 0：
...
也可以在写入配置或恢复配置成功后重启设备：

```sh
./l4_config_file -s cfg.json -H
./l4_config_file -r -H
```
...
程序要求 `-g`、`-s`、`-r` 三个配置动作最多只能指定一个，避免一次命令同时执行多个配置文件操作。`-H` 可以单独执行，也可以和 `-s`、`-r` 组合，但不能和 `-g` 组合；组合使用时仅在配置动作成功后重启。`-R` 只改变执行端，`-S` 只能和 `-R` 一起使用。

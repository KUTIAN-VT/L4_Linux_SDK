...
这里 `-t 60` 表示设备侧最多对频 60 秒，`-w 65` 表示本程序最多等 65 秒的结果事件。一般 `-w` 比 `-t` 多几秒，给事件回调留一点返回时间。
...
```text
waiting for pair result... 1/105
waiting for pair result... 2/105
...
```
...
`-t` 是发给设备的对频超时时间。`-w` 是这个 PC 程序等待事件回调的时间。

如果 `-w` 太短，可能设备还在对频，但程序已经等不下去并发送停止命令。因此一般让 `-w` 比 `-t` 多几秒。
